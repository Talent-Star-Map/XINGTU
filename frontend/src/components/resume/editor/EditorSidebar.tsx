...
export function EditorSidebar({
  ...
  const existingTypes = callbacks.existingSectionTypes
      ...
      <div className="p-3 space-y-5">
        ...
        <div>
          ...
          <div className="space-y-0.5">
            {sorted.map((s) => {
              const Icon = SECTION_ICONS[s.type] || Plus
              ...
              return (
                ...
                >
                  ...
                  <span className="truncate">{s.title || SECTION_LABELS[s.type] || s.type}</span>
                </button>
              )
            })}
            {sorted.length === 0 && (
        ...
        <div>
          ...
          <div className="space-y-0.5">
            ...
            {/* 简历默认模块也允许重复添加(虽然一般不会) */}
            {RESUME_MODULE_TYPES.filter(t => !existingTypes.has(t)).length > 0 && (
              <>
                <div className="my-2 border-t" style={{ borderColor: 'var(--color-outline-variant)' }} />
                {RESUME_MODULE_TYPES.filter(t => !existingTypes.has(t)).map((type) => {
                  const Icon = SECTION_ICONS[type]
                  return (
                    <button
                      key={type}
