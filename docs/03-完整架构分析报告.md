...
gitGraph
    ...
    commit id: "feat: deerflow integration"
    branch feature/crawler
    checkout feature/crawler
    commit id: "feat: job crawler"
    checkout develop
    merge feature/crawler
    branch feature/jobs-api
    checkout feature/jobs-api
    commit id: "feat: jobs CRUD"
    checkout develop
    merge feature/jobs-api
    checkout develop
    merge feature/agent
    branch feature/match-api
    checkout feature/match-api
    commit id: "feat: match engine"
    checkout develop
    merge feature/match-api
    checkout main
    merge develop tag: "v1.0.0-alpha"
```

**推荐策略**: GitHub Flow (简单，适合小团队)
...
gantt
    ...
    section Day 1-2: 项目理解
    阅读 README + AGENTS.md      :d1, 2026-07-14, 0.5d
    阅读 docs/* 设计文档         :d2, after d1, 0.5d
    Docker Compose 启动项目       :d3, after d2, 0.5d
    创建测试账号体验完整流程       :d3_2, after d3, 0.5d

    section Day 3-4: 后端掌握
    阅读 main.py + 启动流程      :d4, after d3_2, 0.5d
    阅读 database.py 数据模型     :d5, after d4, 0.5d
    阅读 auth.py 认证逻辑         :d6, after d5, 0.5d
    Python-FastAPI 调试练习       :d7, after d6, 0.5d

    section Day 5-6: 前端掌握
    阅读 main.tsx + App.tsx       :d8, after d7, 0.5d
    阅读 JobseekerShell + 各页面   :d9, after d8, 1d
    理解 Mock 数据 vs API 调用     :d10, after d9, 0.5d

    section Day 7+: 开发
    分配小任务开始开发             :d11, after d10, 2d
```
...
| 任务 | 耗时 | 产出 |
|------|------|------|
| 阅读 README.md | 15min | 知道项目是干什么的 |
| 阅读 AGENTS.md | 10min | 了解关键注意事项 |
| 阅读 docs/00-基本信息.md | 10min | 了解团队分工 |
| 阅读 docs/02-系统架构设计.md (前 3 节) | 30min | 理解目标架构 |
| 运行 `docker compose up -d` | 10min | 本地启动项目 |
| 访问 http://localhost:3000 体验 | 20min | 熟悉所有页面 |
| 注册账号、登录 | 10min | 体验认证流程 |

**Day 1 验收**: 能回答"这个项目做什么的"、"用了什么技术"、"项目怎么启动"

### Day 2: 架构理解

| 任务 | 耗时 | 产出 |
|------|------|------|
| 阅读 docs/01-赛题深度解读与痛点分析.md | 1h | 理解赛题要求和评分标准 |
| 阅读 docs/02-技术选型之DeerFlow的作用.md | 20min | 理解 Agent 定位 |
| 对照 docs/02-系统架构设计.md 第四部分 vs 实际代码 | 30min | 理解"目标 vs 现状"差距 |
| 查看 docker-compose.yml + 两个 Dockerfile | 15min | 理解部署方式 |
| 查看前端的 nginx.conf + vite.config.ts | 10min | 理解代理配置 |

**Day 2 验收**: 能画出架构图，能指出"哪些已实现、哪些未实现"

### Day 3: 后端深入

| 任务 | 耗时 | 产出 |
|------|------|------|
| 阅读 `backend/main.py` 全部 | 10min | 理解入口、CORS、路由注册 |
| 阅读 `backend/database.py` 全部 | 20min | 理解数据模型、JWT、数据库操作 |
| 阅读 `backend/auth.py` 全部 | 30min | 理解认证流程、验证码、密码策略 |
| 用 curl/Postman 测试 API | 20min | 实际调用所有接口 |
| `pip install -r requirements.txt` + `uvicorn main:app --reload` | 10min | 本地启动后端 |

**Day 3 验收**: 能独立新增一个 FastAPI 路由

### Day 4: 前端深入

| 任务 | 耗时 | 产出 |
|------|------|------|
| 阅读 `frontend/src/main.tsx` | 5min | 理解 React 入口 |
| 阅读 `frontend/src/App.tsx` | 15min | 理解路由和状态管理，**注意 demo_token bug** |
| 阅读 `frontend/src/components/ThemeProvider.tsx` | 5min | 理解主题切换 |
| 阅读一个 Shell 文件 (如 JobseekerShell.tsx) | 15min | 理解导航框架 |
| 阅读一个 Page 文件 (如 Dashboard.tsx) | 10min | 理解页面结构 |
| `cd frontend && npm install && npm run dev` | 10min | 本地启动前端 |

**Day 4 验收**: 能修改一个前端页面，理解"硬编码数据在哪里"

### Day 5: 前后端联调

| 任务 | 耗时 | 产出 |
|------|------|------|
| 对比 Login.tsx (有真实 API) vs Dashboard.tsx (纯 Mock) | 30min | 理解两种模式 |
| 阅读 `ProfileEdit.tsx` 中 `?token=` 的调用方式 | 15min | 理解 API 调用模式 |
| 理解 types/index.ts 中的类型定义与 page 中实际使用不匹配的问题 | 15min | 理解类型现状 |
| 阅读 Graph3D.tsx | 30min | 理解 Three.js 图谱实现 |

**Day 5 验收**: 能写一个前端页面调用后端 API

### Day 6: 开发准备

| 任务 | 耗时 | 产出 |
|------|------|------|
| 阅读本项目分析报告 (Sections 1-10) | 30min | 全面理解 |
| 确定分配的任务模块 | 10min | |
| 搭建开发环境 | 20min | |
| 开始第一个开发任务 | 2h | 实际产出 |
