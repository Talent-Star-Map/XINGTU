...
|---|------|--------|------|--------|---------|------|
| ~~**0**~~ | ~~**修复 App.tsx demo_token Bug**~~ | ~~**阳总**~~ | ~~**前端**~~ | ~~**P0**~~ | ~~**0.5d**~~ | ~~**无 ✅ 已完成**~~ |
| 1 | 数据库模型: match_records 表 | 阳总 | 后端 | P0 | 0.5d | 无 |
| 2 | Mock 岗位数据（20条） | 阳总 | 后端 | P0 | 0.5d | 无 |
| 3 | 后端: 匹配引擎 (规则算法) | 洋总 | 后端 | P0 | **3d** | #2 |
| 4 | 后端: 匹配 API (POST /api/match) | 阳总 | 后端 | P0 | 1d | #1, #3 |
| 5 | 后端: 匹配结果 API (GET /api/match/result) | 阳总 | 后端 | P1 | 0.5d | #4 |
| 6 | 后端: 学习路径生成 API | 洋总 | 后端 | P0 | 1d | #4 |
| 7 | 后端: 企业端人才搜索 API（先Mock） | 阳总 | 后端 | P0 | 1.5d | 无 |
| 8 | 匹配 API 单元测试 | 阳总 | 测试 | P1 | 0.5d | #4 |
| 9 | 学习路径生成单元测试 | 洋总 | 测试 | P1 | 0.5d | #6 |
| 10 | 前端: Match.tsx Mock→API 联调 | 洋总 | 前端 | P0 | **1.5d** | #4, #5, #6 |
| 11 | 前端: TalentSearch.tsx Mock→API 联调 | 阳总 | 前端 | P0 | **1.5d** | #7 |
| 12 | 后端: 匹配精度测试 (≥90%) | 洋总 | 测试 | P0 | **2d** | #3 |
| 13 | 错误处理统一封装 | 阳总 | 后端 | P1 | 0.5d | 无 |
| 14 | 匹配模块联调 + 自测 | 阳总+洋总 | 全栈 | P0 | **2d** | #10, #11 |

**总计**: 阳总约 **7.5d**，洋总约 **7d**，联调 **2d** = **约 2.5 周**（增加 0.5 周缓冲）

---

## 三、详细任务拆解

### ~~任务 0：修复 App.tsx demo_token Bug~~ ✅ 已完成

当前 `App.tsx:31-34` 已移除 `demo_token` 覆写逻辑，直接读取 localStorage 中的真实 JWT。无需再改。
...
2. **不再新建 `match_skills` 表，直接用 JSON 存储技能列表**（简化设计）
...
class MatchRecord(Base):
    ...
    skill_score = Column(Integer, default=0)        # 技能维度 (0-25)
    experience_score = Column(Integer, default=0)   # 经验维度 (0-25)
    education_score = Column(Integer, default=0)    # 学历维度 (0-25)
    salary_score = Column(Integer, default=0)       # 薪资维度 (0-25)
    matched_skills = Column(Text, default='')       # JSON: 已掌握的技能列表 ["Python", "FastAPI"]
    missing_skills = Column(Text, default='')       # JSON: 缺失的技能列表 ["LangChain", "RAG"]
    advice = Column(Text, default='')               # 改进建议
    learning_path = Column(Text, default='')        # JSON: 学习路径（可选）
    user_skills = Column(Text, default='')          # JSON: 用户输入的技能
    created_at = Column(DateTime, default=func.now())
```

**设计说明**:
- 去掉原设计中的 `match_skills` 表，直接用 JSON 字段存储技能列表
- 理由：技能列表通常不超过 20 项，JSON 足够；减少一张表，简化查询；前端接收 JSON 直接渲染，无需转换
...
**背景**: 上游岗位数据可能延迟，需要立即准备 Mock 数据，不阻塞匹配引擎开发。

**操作步骤**:
1. 在数据库中插入 20 条岗位数据（含技能、薪资、经验要求）
2. 或在匹配引擎中使用硬编码 Mock 数据（先跑通算法）

**Mock 数据示例**（存放在 `backend/mock_data/jobs.py`）:
...
MOCK_JOBS = [
    ...
    },
    {
        ...
        "title": "大模型算法工程师",
        "company": "腾讯",
        "skills": ["Python", "PyTorch", "LLM", "NLP", "CUDA"],
        "exp_required": "3-5年",
        "edu_required": "硕士",
        "salary_min": 35,
        "salary_max": 60
...
**背景**: 这是匹配模块的**核心逻辑**。当前前端展示的 72 分是硬编码，需要替换为真实算法。**建议时间从 2d 调整为 3d**，需调试边界情况、处理异常输入、写单元测试。
...
**背景**: 确保匹配 API 接口正确性，避免后期返工。

**代码位置**: 新建 `backend/tests/test_match_api.py`
...
def test_match_api_success():
    ...
    assert 0 <= data["data"]["match_score"] <= 100

def test_match_api_missing_fields():
    """测试缺少必填字段"""
    response = client.post("/api/match", json={})
...
**代码位置**: 新建 `backend/tests/test_learning_path.py`

**测试内容**:
```python
from services.learning_path import generate_learning_path

def test_generate_learning_path_with_skills():
    """测试有缺失技能时的学习路径生成"""
...
### 任务 10：前端 Match.tsx 联调（洋总）

**目标**: 将硬编码数据替换为真实 API 调用。**建议时间从 1d 调整为 1.5d**，联调往往有坑。
...
### 任务 11：前端 TalentSearch.tsx 联调（阳总）
...
**背景**: 赛题要求人岗匹配准确率 ≥ 90%（硬性指标）。**建议时间从 1.5d 调整为 2d**，需构造高质量测试数据。
...
**背景**: 所有 API 应返回统一错误格式，便于前端处理。

**代码位置**: 新建 `backend/utils/error_handler.py`
...
def create_error_response(code: int, message: str, details: Any = None) -> dict:
    """统一错误响应格式"""
    return {
        "success": False,
        "error": {
...
**在路由中使用**:
```python
from utils.error_handler import AppException, create_error_response
...
async def match(body: MatchRequest):
    try:
        ...
        raise AppException(code=400, message="参数错误", details=str(e))
```
